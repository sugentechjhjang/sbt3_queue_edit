...
#endif //BLCOK_SOURCE_OLD
  /* 기존 방법으로 IAP Mode로 원하지 않는 상황에 들어오게 됨 해당 방법을 수정 */  
  Delay_ms(300);
//  SerialPutString("\r\nPassword: ");
//  SerialPutString("\r\nPW ");
  SerialPutString("PW");
...
//  Allo_S_GPIO_Init();
  /* UART1 HW Set */
  vUART1_Set();
//  USART3_Init();  // 설정 안해주면은 x axis 이상 동작 함. 이유는 분석중....
}
...
* Description    : IAP의 클럭 을 설정 하는 함수 
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : USART1의 GPIO Pin 설정
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Input          : None
* Output         : None
* Return         : dwReturnData = 1: IAP 진입, 0 이면 APP 살행 
*******************************************************************************/
uint32_t dwIAP_InputMode_Check(void)
{
...
//  vBuzzer_OnOff(nOff);
  ...
  while(1)
  {
    ...
    {
      dwGetCnt++;
      if(dwGetCnt == 9 ) break; //'K', 'M', 'A', 'C', '1', '7', '1', '7', '0'// '2', '0', '1', '2', '0', '2', '5', '7', '8'  --> 9개 의 Key 
    }
    else
    {
      timeCnt++;
  //   if(dwCnt > 6000000 )break; // 0xCDFE600 == 216000000 --> 4-5초
      if(timeCnt > 1000000 )break; // 0xCDFE600 == 216000000 --> 4-5초
    }
  }
